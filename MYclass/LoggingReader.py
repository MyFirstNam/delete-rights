...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Flask NodeSimulation Startup Configuration')
...
def get_operation_log():
    try:
        ...
        # 第一种情况，infoID和time_str都不为空，返回此信息特定时间的数据
        if infoID and time_str:
...
#########################删除通知解析#########################
        ...
        print(f"Parent NodeSimulation: {parentNode}")
        print(f"Root NodeSimulation: {rootNode}")
        print(f"Other NodeSimulation: {otherNode}")
