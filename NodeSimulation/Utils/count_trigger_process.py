...
def count_tri_pro(delintention, count_limit, bus_id):
    """工作原理：
    具体的事件处理函数在解析数据包后，判定结果为计次触发
    但是当删除指令通知与确认系统接收到请求删除的数据包时，表明计次已经结束，开始删除通知等后续操作
    此函数的功能时：
        解析计次删除意图，生成删除请求
        存储当前信息，因为计次信息在后续操作中不会被保留，所以当前进行存储
    ...
    try:
        ...
        if is_database_exists(host, user, password, port, database):
            ...
            # 记录删除意图数据
            loggingwriter.count_record(delintention["affairs_id"], delintention["info_id"], IsRoot,
                                       delintention["user_id"], delintention["deleteMethod"],
                                       delintention["deleteGranularity"], TriggerType,
                                       delintention["source_bus_id"], Count_limit)
...
# }
# Count_limit = 5
# db = count_tri_pro(delintention, Count_limit, "b5555")
